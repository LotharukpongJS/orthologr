...
the these programming languages:
 ...
 For Windows users, see the [Setting up DIAMOND2 and BLAST+ on Windows](setup_windows.html#install-rtools) vignette for details on installing a C++ compiler (Rtools).
...
-> Please carefully read the `Environment Variables` section of the `installation manual: Windows` and make sure the execution `PATH` variable is set correctly. For Windows users, see also the [Setting up DIAMOND2 and BLAST+ on Windows](setup_windows.html) vignette.
...
1) Go to the download site in the [`DIAMOND2` wiki](https://github.com/bbuchfink/diamond/wiki/2.-Installation) and follow the instructions for installation. `DIAMOND2` is supported on Linux, macOS and Windows. For Windows users, see also the [Setting up DIAMOND2 and BLAST+ on Windows](setup_windows.html) vignette.
